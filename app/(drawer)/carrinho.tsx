// Carrinho.tsx - Versão React Native com Imagens Corrigidas
...
// ===== IMPORTANDO AS IMAGENS COM require (JEITO CERTO NO RN) =====
const imagemSertao = require('../../assets/images/8665699427_16c427ccd5_z.jpg');
...
const Carrinho: React.FC = () => {
  ...
  const [produtos, setProdutos] = useState<Produto[]>([
    {
      id: 1,
      nome: 'Pacote "Cultura e Tradição no Sertão" – Pernambuco',
      descricao: 'Explore a rica cultura do sertão pernambucano com guias locais',
      localizacao: 'Região do Rio Pajeú, próximo a Serra Talhada – PE',
      preco: 3200,
      imagem: imagemSertao,
      checkin: '15/12/2024',
      checkout: '20/12/2024'
  ...
  const [cupomFeedback, setCupomFeedback] = useState({ mensagem: '', tipo: '' });

  const recomendacoes: Recomendacao[] = [
    { id: 1, nome: 'Caneca Personalizada', preco: 39.90, imagem: canecaImage },
    { id: 2, nome: 'Porta-retrato vídeo', preco: 49.90, imagem: portaRetratoImage },
    { id: 3, nome: 'Chaveiro com TAG', preco: 19.90, imagem: chaveiroImage },
    { id: 4, nome: 'Blusa Personalizada', preco: 59.90, imagem: blusaImage },
  ];
  ...
  const calcularTotal = (): number => {
    ...
    produtos.forEach(prod => {
      total += prod.preco * (quantidades[prod.id] || 1);
    });
    return total;
  };
  ...
  const atualizarQuantidade = (id: number, incremento: number) => {
    setQuantidades(prev => {
      const atual = prev[id] || 1;
      const novo = Math.max(1, atual + incremento);
      return { ...prev, [id]: novo };
    });
  };
  ...
  const aplicarCupom = () => {
    ...
    if (codigo === 'IMERCIA10') {
      ...
      setDesconto(10);
      setCupomFeedback({ mensagem: 'Cupom aplicado com sucesso! 10% de desconto', tipo: 'success' });
      setCupom('');
    } else if (codigo === 'IMERCIA20') {
      setCupomAplicado(codigo);
      setDesconto(20);
      setCupomFeedback({ mensagem: 'Cupom aplicado com sucesso! 20% de desconto', tipo: 'success' });
      setCupom('');
    } else if (codigo === '') {
      setCupomFeedback({ mensagem: 'Digite um código de cupom', tipo: 'error' });
    } else {
      setCupomFeedback({ mensagem: 'Código inválido. Tente novamente.', tipo: 'error' });
    }
        ...
        <View style={styles.gridContainer}>
          ...
          <View style={styles.produtosSection}>
            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitle}>Produtos ({produtos.length})</Text>
              {produtos.length > 0 && (
                <TouchableOpacity onPress={() => setProdutos([])} style={styles.btnLimpar}>
            ...
            {produtos.length === 0 ? (
              <View style={styles.carrinhoVazio}>
                ...
                <Text style={styles.vazioSubtitle}>Que tal explorar nossas ofertas?</Text>
                <TouchableOpacity style={styles.btnContinuarComprando} onPress={() => router.push('/')}>
                  <Text style={styles.btnContinuarComprandoText}>Continuar Comprando</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <>
                {produtos.map((produto) => (
                  <View key={produto.id} style={styles.produtoCard}>
                    <View style={styles.produtoImagem}>
                      <Image source={produto.imagem} style={styles.produtoImg} />
                    </View>
                    
                    <View style={styles.produtoInfo}>
                      <Text style={styles.produtoNome}>{produto.nome}</Text>
                      <Text style={styles.produtoDescricao}>{produto.descricao}</Text>
                      <Text style={styles.produtoLocalizacao}> {produto.localizacao}</Text>
                      <Text style={styles.produtoDatas}>
                         {produto.checkin} até {produto.checkout}
                      </Text>
                      
                      <View style={styles.produtoActions}>
                        <View style={styles.produtoQuantidade}>
                          <TouchableOpacity
                            onPress={() => atualizarQuantidade(produto.id, -1)}
                            style={[styles.qtdBtn, quantidades[produto.id] <= 1 && styles.qtdBtnDisabled]}
                            disabled={quantidades[produto.id] <= 1}
                          >
                            <Text style={styles.qtdBtnText}>−</Text>
                          </TouchableOpacity>
                          <Text style={styles.qtdNumero}>{quantidades[produto.id] || 1}</Text>
                          <TouchableOpacity
                            onPress={() => atualizarQuantidade(produto.id, 1)}
                            style={styles.qtdBtn}
                          >
                        ...
                        <View style={styles.produtoPreco}>
                          <Text style={styles.precoTotal}>
                            R$ {(produto.preco * (quantidades[produto.id] || 1)).toFixed(2).replace('.', ',')}
                          </Text>
                        </View>
                      </View>
                    </View>
            ...
            <View style={styles.recomendacoes}>
              <Text style={styles.recomendacoesTitle}> Você também pode gostar</Text>
              <View style={styles.recomendacoesGrid}>
                {recomendacoes.map(item => (
                  <TouchableOpacity key={item.id} style={styles.recomendacaoCard}>
                    <Image source={item.imagem} style={styles.recomendacaoImagem} />
                    <Text style={styles.recomendacaoNome}>{item.nome}</Text>
                    <View style={styles.recomendacaoPreco}>
                      <Text style={styles.recomendacaoPrecoText}>R$ {item.preco.toFixed(2).replace('.', ',')}</Text>
          ...
          <View style={styles.resumoSection}>
            ...
            <View style={styles.resumoItens}>
              <View style={styles.resumoLinha}>
                ...
                <Text style={styles.resumoLinhaValor}>R$ {calcularTotal().toFixed(2).replace('.', ',')}</Text>
              </View>
              ...
              <View style={styles.resumoCupom}>
                <View style={styles.cupomInputWrapper}>
                  ...
                  />
                  <TouchableOpacity
                    style={[styles.btnAplicarCupom, cupomAplicado && styles.btnAplicarCupomDisabled]}
                ...
                {cupomFeedback.mensagem && (
                  <Text style={[styles.cupomFeedback, cupomFeedback.tipo === 'success' ? styles.cupomFeedbackSuccess : styles.cupomFeedbackError]}>
                    {cupomFeedback.mensagem}
                  </Text>
                )}
                ...
                {cupomAplicado && (
                  <View style={styles.cupomAplicado}>
                    <Text style={styles.cupomAplicadoText}>{cupomAplicado} - {desconto}% OFF</Text>
              ...
              {desconto > 0 && (
                <View style={[styles.resumoLinha, styles.resumoLinhaDesconto]}>
                  ...
                  <Text style={styles.resumoLinhaValor}>- R$ {(calcularTotal() * desconto / 100).toFixed(2).replace('.', ',')}</Text>
                </View>
              )}
            ...
            <View style={styles.resumoAcoes}>
              <TouchableOpacity
                style={[styles.btnFinalizar, produtos.length === 0 && styles.btnFinalizarDisabled]}
              ...
              <TouchableOpacity style={styles.btnContinuar} onPress={() => router.push('/')}>
            ...
            <View style={styles.resumoSeguranca}>
              ...
              <Text style={styles.resumoSegurancaText}>🔄 Pagamento protegido</Text>
            </View>
          </View>
        </View>
      </ScrollView>
